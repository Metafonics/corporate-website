(function ($) {
	...
	if ($('.tabs-box').length) {
		$('.tabs-box .tab-buttons .tab-btn').on('click', function (e) {
			...
			if ($(target).is(':visible')) {
				...
				// Tab'ı göster ve tam olarak görünür olduğunda carousel'ı başlat
				$(target).fadeIn(300, function() {
					$(target).addClass('active-tab');
					var carousel = target.find('.price-carousel');
					if (carousel.length && window.initPriceCarousel) {
						// Biraz daha bekle ki CSS transitions tamamlansın
						setTimeout(function() {
							window.initPriceCarousel(carousel[0]);
						}, 50);
					}
				});
			}
	...
	window.initPriceCarousel = function(container) {
		if (!container) return;

		// Container için unique bir ID oluştur
		var containerId = container.getAttribute('data-swiper-id');
		if (!containerId) {
			containerId = 'swiper-' + Math.random().toString(36).substr(2, 9);
			container.setAttribute('data-swiper-id', containerId);
		}

		// Eğer bu container için bir slider varsa önce yok et
		if (priceSliders[containerId]) {
			priceSliders[containerId].destroy(true, true);
			delete priceSliders[containerId];
		}

		// Toplam slide sayısını kontrol et
		var slideCount = container.querySelectorAll('.swiper-slide').length;
		var nextBtn = container.querySelector('.price-carousel-button-next');
		var prevBtn = container.querySelector('.price-carousel-button-prev');

		// 3'ten fazla paket varsa butonları göster, değilse gizle
		if (slideCount > 3) {
		...
		var slider = new Swiper(container, {
			...
			on: {
				init: function() {
					// Swiper başlatıldığında layout'u güncelle
					this.update();
					// Carousel'ı görünür yap
					setTimeout(function() {
		...
		priceSliders[containerId] = slider;
		return slider;
	}

	// İlk yüklemede aktif tab'daki carousel'ı başlat
	// Sayfa tamamen yüklendikten sonra başlat
	$(window).on('load', function() {
		setTimeout(function() {
			if ($('.tabs-box .tab.active-tab .price-carousel').length) {
				$('.tabs-box .tab.active-tab .price-carousel').each(function() {
					window.initPriceCarousel(this);
				});
			}
		}, 100);
	});
	...
	window.showCenterToast = function (message, type = 'error', duration = 2500) {
		...
		// Eğer aynı mesaj zaten gösteriliyorsa tekrar ekleme
